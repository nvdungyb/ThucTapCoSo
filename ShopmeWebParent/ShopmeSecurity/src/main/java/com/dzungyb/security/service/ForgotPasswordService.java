...
public class ForgotPasswordService {
    ...
    public ForgotPasswordService(MailService mailService, RedisService redisService, UserService userService) {
    ...
    public boolean verifyAuthCode(String email, String authCode) {
        ...
        if (savedAuthCode == null) {
            logger.warn("Auth code not found or expired: {}", MailService.maskEmail(email));
            return false;
        }

        boolean isValid = constantTimeEquals(savedAuthCode, authCode);
        if (!isValid) {
            logger.warn("Invalid auth code: {}", MailService.maskEmail(email));
        }
    ...
    private boolean isValidResetToken(String email, String resetToken) {
        ...
        if (savedToken == null || !savedToken.equals(resetToken)) {
            logger.warn("Invalid or expired reset token for email: {}", MailService.maskEmail(email));
            return false;
        }
        return true;
    }
    ...
    private void updatePasswordAndInvalidateToken(String email, String newPassword) throws FailedToUpdatePasswordException {
        try {
            ...
            logger.error("Error updating password for email: {}", MailService.maskEmail(email), ex);
